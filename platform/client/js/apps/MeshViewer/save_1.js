...
class MeshViewer {
	...
	init(params) {
		...
		this.render_form();
		this.render_info();
		let ele_update_label = document.getElementById("update_label")
		ele_update_label.addEventListener("click", this.on_update_label.bind(this));
	}
	...
	render_form() {
		...
		ReactDOM.render(<FormUI sentences_length={this.sentences_length} scene_id={this._scene_id} object_id={this._object_id} object_name={this._object_name} ann_id={this._anno_id} description={this._description} position={this._position} labeled_id={this.selected_id} labeled_phrase={this.labeled_phrase}/>, form_container, function() {
	...
	render_info(results_list) {
		...
		this._scene_id = window.location.href.split("&scene_id=")[1];
		ReactDOM.render(<InfoUI _scene_id={this._scene_id}/>, info_container, function() {
	...
	on_get_labeled_data() {
		...
		window.xhr_json("GET", path.join("/apps/database/phrasegrounding/hasLabeled/", _scene_id_for_check, _scene_obj_id_for_check)).then(results => {
			...
			this.sentences_length = '已经标注了第' + _has_labeled + '句, 共计' + _has_labeled_length + ' / ' + _all_sentences;
		})
	}
	on_update_label() {
		let _label = {
	...
	on_label_operation(id) {
		return function() {
			...
			// set view
			this.highlight(operation, object_id, anno_id);
